...
def itr_predict(data, theta, beta, binary):
    """ Give predictions to data. Give binary prediction if binary == True.
...
def main():
    ...
    # it_lst = [10, 15, 25, 40, 60, 85]
    # # Tune lr:
    # for lr in lr_lst:
    #     print("Learning Rate: {}".format(lr))
    #     irt(train_data, val_data, lr, 20)
    # # Tune # iteration:
    # for it in it_lst:
    #     print("# iterations: {}".format(it))
    #     irt(train_data, val_data, 0.01, it)
    chosen_lr = 0.01
    chosen_it = 15
    theta, beta, train_acc_lst, train_nllk_lst, val_acc_lst, val_nllk_lst = \
        irt(train_data, val_data, chosen_lr, chosen_it)
    # plt.plot(train_nllk_lst, color="blue", label="train")
    # plt.plot(val_nllk_lst, color="red", label="val")
    # plt.xlabel("iteration")
    # plt.ylabel("(negative) log-likelihood")
    # plt.title("Training&Validation Negative log-likelihoods Against Iteration")
    # plt.legend()
    # plt.show()
    ...
    for question in questions:
        plt.plot(theta, sigmoid(theta - beta[question]),
